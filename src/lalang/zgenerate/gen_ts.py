...
class Generator:
    ...
    def function_item(self, cucu, returning=True):
        ...
        if fnconfig and "arrow" in fnconfig:
            ...
            # print('#2', fnconfig, type(fnconfig))
            if "iife" in fnconfig:
                fnconfig.remove("iife")
                # print('#3', fnconfig, type(fnconfig))
                if fnconfig:
    ...
    def block_item(self, tree):
        ...
        for cucu in anak(tree):
            if data(cucu) == "function_item":
                kembali += self.function_item(cucu)
            elif data(cucu) == "class_item":
                kembali += self.class_item(cucu)
            elif data(cucu) == "interface_item":
                kembali += self.interface_item(cucu)
            elif data(cucu) == "scope_item":
                kembali += self.scope_item(cucu)
        # if kembali:
        #   self.print(kembali)
        return kembali
    ...
    def searchable(self, tree, filepath="data/import_ts.txt"):
        """
        searchable
          searchable_kw
    ...
    def import_item(self, tree):
        ...
        for item in anak(tree):
            if data(item) == "import_container":
                ...
                search_mode = True
                filepath = joiner(here(__file__), "../data/import_ts.txt")
              ...
              statement_item
                single_statement
                  stdout_operation
                    ...
                    console_log
                      expression_item
                        nama_identifier     hello
        """
        kembali = ""
        for item in anak(tree):
            jenis = data(item)
            if jenis == "scope_config":
                pass
            elif jenis == "condition_body":
                kembali += self.condition_body(item, returning=True)
        print(f"kembalian scope_item:", kembali)
        return kembali
    ...
    def do_generate(self):
        ...
        if self.root.data == "item":
            for dahan in anak(self.root):
                print("gen_ts do _generate:", data(dahan))
                if data(dahan) == "block_item":
                    kembali += self.block_item(dahan)
                elif data(dahan) == "statement_item":
                    self.statement_item(dahan)
                elif data(dahan) == "import_item":
                    kembali += self.import_item(dahan)
                elif data(dahan) == "export_item":
                    kembali += self.export_item(dahan)
