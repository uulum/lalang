...
class Generator:
    ...
    def enum_member(self, tree):
        ...
        for item in anak(tree):
            if data(item) == "nama_identifier":
                nama = token(
                    item
                ).upper()  # https://stackoverflow.com/questions/75759/how-to-implement-enums-in-ruby
    ...
    def statement_item(self, contentitem, returning=True):
        ...
        if jenisstmt.data == "single_statement":
            ...
            jika_mintaresult = self.instantiation_expression(
                jenisstmt, returning=returning
            )
            if returning:
                return jika_mintaresult
    ...
    def field_content(self, tree):
        ...
        def_initialize = ruby_struct_template.replace(
            "__PARAMLIST__", __PARAMLIST__
